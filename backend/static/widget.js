(function () {
    ...
    branded: {
      ...
      panelCSS: `
        .gk-panel{position:fixed;${pos};bottom:94px;z-index:99998;width:340px;
          ...
          box-shadow:0 12px 56px rgba(${rgb},.4);
        ...
        .gk-panel.open{transform:scale(1) translateY(0);opacity:1;pointer-events:all}
        .gk-header{background:rgba(0,0,0,.15);padding:16px 18px 14px;display:flex;align-items:center;justify-content:space-between;flex-shrink:0;border-bottom:1px solid rgba(255,255,255,.1)}
        .gk-htitle{font-size:15px;font-weight:700;color:${cfg.textColor}}
        .gk-honline{font-size:11px;color:rgba(255,255,255,.65);margin-top:1px}
        .gk-hclose{background:rgba(255,255,255,.15);border:none;border-radius:50%;width:28px;height:28px;display:flex;align-items:center;justify-content:center;cursor:pointer;color:${cfg.textColor};flex-shrink:0;transition:background .15s}
        .gk-hclose svg{width:13px;height:13px}
        .gk-hclose:hover{background:rgba(255,255,255,.28)}
        .gk-msgs{flex:1;overflow-y:auto;display:flex;flex-direction:column;gap:10px;padding:16px;background:rgba(0,0,0,.12);min-height:160px}
        .gk-msgs::-webkit-scrollbar{width:3px}
        .gk-msgs::-webkit-scrollbar-thumb{background:rgba(255,255,255,.2);border-radius:2px}
        .gk-msg.b .gk-bub{background:rgba(255,255,255,.18);border-radius:12px 12px 12px 2px;color:${cfg.textColor};backdrop-filter:blur(8px);border:1px solid rgba(255,255,255,.12)}
        .gk-msg.u .gk-bub{background:rgba(0,0,0,.2);border-radius:14px 14px 2px 14px;color:${cfg.textColor}}
        .gk-time{color:rgba(255,255,255,.35)}
        .gk-typing .gk-dot{background:rgba(255,255,255,.45)}
        .gk-welcome p{color:rgba(255,255,255,.75)}
        .gk-welcome-icon{background:rgba(255,255,255,.15);color:${cfg.textColor}}
        .gk-chips .gk-chip{border-color:rgba(255,255,255,.35);background:rgba(255,255,255,.12);color:${cfg.textColor}}
        .gk-chips .gk-chip:hover{background:rgba(255,255,255,.25)}
        .gk-footer{padding:10px 12px;background:rgba(0,0,0,.15);border-top:1px solid rgba(255,255,255,.1);display:flex;gap:8px;align-items:center;flex-shrink:0}
        .gk-input{flex:1;border:none;border-radius:22px;padding:10px 16px;font-size:13.5px;outline:none;color:#111;background:rgba(255,255,255,.92);transition:background .15s}
        .gk-input:focus{background:#fff}
        .gk-input::placeholder{color:#aaa}
        .gk-send{width:36px;height:36px;border-radius:50%;border:none;background:rgba(255,255,255,.25);cursor:pointer;display:flex;align-items:center;justify-content:center;color:${cfg.textColor};flex-shrink:0;transition:background .15s,transform .12s}
        .gk-send:hover{background:rgba(255,255,255,.38);transform:scale(1.05)}
        .gk-send:disabled{opacity:.35;cursor:not-allowed;transform:none}
        .gk-send svg{width:15px;height:15px}
        .gk-brand{text-align:center;font-size:10px;color:rgba(255,255,255,.35);padding:4px 0 8px;flex-shrink:0}
        .gk-brand a{color:rgba(255,255,255,.35);text-decoration:none}`,
      buildBubble: () => `<button class="gk-bubble" aria-label="Open chat">
        <span class="icon-chat" style="display:flex">${svgChat}</span>
        <span class="icon-close" style="display:flex">${svgClose}</span>
      </button>`,
  ...
  function mount() {
    ...
    if (cfg.mode === 'inline') {
      ...
      // Inline uses the selected theme but rendered as a static embedded panel
      // Add inline-specific CSS overrides
      const inlineStyle = document.createElement('style');
      inlineStyle.textContent = `
        .gk-inline-wrap{position:relative;width:100%}
        .gk-inline-wrap .gk-panel{
          position:static!important;transform:none!important;opacity:1!important;
          pointer-events:all!important;width:100%!important;max-height:480px;
          border-radius:12px!important;display:flex!important;
          box-shadow:0 2px 20px rgba(0,0,0,.08)!important;
        }
        .gk-inline-wrap .gk-msgs{min-height:200px}
        .gk-inline-wrap .gk-overlay{display:none!important}
        .gk-inline-wrap .gk-hclose{display:none!important}
      `;
      document.head.appendChild(inlineStyle);

      wrap.className   = 'gk-inline-wrap';
      wrap.innerHTML   = theme.buildPanel();
      ...
      // Inline is always open — no toggle needed
      isOpen = true;
    } else {
